...
public class EntityRapace extends EntityAnimal {
    ...
    /**
     * The rate at which we will travel.
     */
    private static final float SPEED = .25F;

    /**
     * Whether our navigator has been overwritten yet.
     */
    private boolean hasSetNav = false;

    /**
     * Creates a new EntityRapace object.
     *
     * @param  par1World  DOCUMENT ME!
     */
    ...
    public void setPosition(double par1, double par3, double par5) {
        ...
        if (!hasSetNav) {
            ...
            try {
                Field navigatorField = EntityLiving.class.getDeclaredField(
                        "navigator" /* NOI18N */);
                navigatorField.setAccessible(true);
                navigatorField.set(this, new FlyingPathNavigate(this, worldObj, 16));
                hasSetNav = true;
    ...
    //<editor-fold defaultstate="collapsed" desc="EntityFlying stuff">
    /**
     * Called when the mob is falling. Calculates and applies fall damage.
     *
     * @param  par1  DOCUMENT ME!
     */
     ...
     *
     * @return  true if this entity is by a ladder, false otherwise.
     */
    ...
    //<editor-fold defaultstate="collapsed" desc="FlyingPathNavigate">
    /**
     * An entire class copied directly from PathNavigate for the sole purpose of
     * allowing flight.
     */
    private static class FlyingPathNavigate extends PathNavigate {
        ...
        /**
         * Coordinates of the entity's position last time a check was done (part
         * of monitoring getting 'stuck').
         */
        private Vec3 lastPosCheck = Vec3.createVectorHelper(0.0D, 0.0D, 0.0D);

        /**
         * Specifically, if a wooden door block is even considered to be
         * passable by the pathfinder.
         */
        ...
        /**
         * If the entity can swim. Swimming AI enables this and the pathfinder
         * will also cause the entity to swim straight upwards when underwater
         */
        private boolean canSwim = false;

        public FlyingPathNavigate(EntityLiving par1EntityLiving, World par2World, float par3) {
         ...
         *
         * @param   par1  DOCUMENT ME!
         * @param   par3  DOCUMENT ME!
         * @param   par5  DOCUMENT ME!
         *
        ...
        public PathEntity getPathToXYZ(double par1, double par3, double par5) {
            return (!this.canNavigate())
                ? null
                : this.worldObj.getEntityPathToXYZ(this.theEntity, MathHelper.floor_double(par1),
                    (int) par3, MathHelper.floor_double(par5), this.pathSearchRange,
         ...
         * @param   par3  DOCUMENT ME!
         * @param   par5  DOCUMENT ME!
         * @param   par7  DOCUMENT ME!
         *
         * @return  try to find and set a path to XYZ.
         */
        public boolean tryMoveToXYZ(double par1, double par3, double par5, float par7) {
            PathEntity var8 = this.getPathToXYZ((double) MathHelper.floor_double(par1),
                    (double) ((int) par3), (double) MathHelper.floor_double(par5));
         ...
         *
         * @param   par1EntityLiving  DOCUMENT ME!
         *
         * @return  the path to the given EntityLiving.
         */
        public PathEntity getPathToEntityLiving(EntityLiving par1EntityLiving) {
            return (!this.canNavigate())
                ? null
                : this.worldObj.getPathEntityToEntity(this.theEntity, par1EntityLiving,
                    this.pathSearchRange, this.canPassOpenWoodenDoors,
         ...
         * @param   par2              DOCUMENT ME!
         *
         * @return  try to find and set a path to EntityLiving.
         */
        ...
        /**
         * sets the active path data if path is 100% unique compared to old
         * path, checks to adjust path for sun avoiding ents and stores end
         * coords.
         *
         * @param   par1PathEntity  DOCUMENT ME!
         * @param   par2            DOCUMENT ME!
         *
         * @return  sets the active path data if path is 100% unique compared to
         *          old path, checks to adjust path for sun avoiding ents and
         *          stores end coords.
         */
         ...
         *
         * @return  gets the actively used PathEntity.
         */
        ...
        private void pathFollow() {
            ...
            for (int var3 = this.currentPath.getCurrentPathIndex();
                    var3 < this.currentPath.getCurrentPathLength(); ++var3) {
            ...
            for (var4 = this.currentPath.getCurrentPathIndex(); var4 < var2; ++var4) {
                ...
                if (var1.squareDistanceTo(
                            this.currentPath.getVectorFromIndex(this.theEntity, var4)) <
                        (double) var8) {
            ...
            for (int var7 = var2 - 1; var7 >= this.currentPath.getCurrentPathIndex(); --var7) {
                ...
                if (this.isDirectPathBetweenPoints(var1,
                            this.currentPath.getVectorFromIndex(this.theEntity, var7), var4, var5,
                            var6)) {
         ...
         *
         * @return  if null path or reached the end.
         */
        ...
        private Vec3 getEntityPosition() {
            return Vec3.getVec3Pool()
                    .getVecFromPool(this.theEntity.posX, (double) this.getPathableYPos(),
                        this.theEntity.posZ);
        }

        /**
         * Gets the safe pathing Y position for the entity depending on if it
         * can path swim or not.
         *
         * @return  gets the safe pathing Y position for the entity depending on
        ...
        private int getPathableYPos() {
            ...
            if (this.theEntity.isInWater() && this.canSwim) {
                ...
                int var2 = this.worldObj.getBlockId(MathHelper.floor_double(this.theEntity.posX),
                        var1, MathHelper.floor_double(this.theEntity.posZ));
                int var3 = 0;
                ...
                do {
                    ...
                    ++var1;
                    var2 = this.worldObj.getBlockId(MathHelper.floor_double(this.theEntity.posX),
                            var1, MathHelper.floor_double(this.theEntity.posZ));
                    ++var3;
                } while (var3 <= 16);
         ...
         *
         * @return  if on ground or swimming and can swim.
         */
        private boolean canNavigate() {
            return !this.isInFluid() || this.canSwim;
        }

        /**
         * Returns true if the entity is in water or lava, false otherwise.
         *
         * @return  true if the entity is in water or lava, false otherwise.
         */
         ...
         *
         * @param   par1Vec3  DOCUMENT ME!
         * @param   par2Vec3  DOCUMENT ME!
         * @param   par3      DOCUMENT ME!
         * @param   par4      DOCUMENT ME!
         * @param   par5      DOCUMENT ME!
         *
         * @return  true when an entity of specified size could safely walk in a
         *          straight line between the two points.
         */
        private boolean isDirectPathBetweenPoints(Vec3 par1Vec3,
                                                  Vec3 par2Vec3,
                                                  int  par3,
                                                  int  par4,
                                                  int  par5) {
            ...
            if (var12 < 1.0E-8D) {
                ...
                if (!this.isSafeToStandAt(var6, (int) par1Vec3.yCoord, var7, par3, par4, par5,
                            par1Vec3, var8, var10)) {
                    return false;
                } else {
                    par3 -= 2;
                    ...
                    } while (this.isSafeToStandAt(var6, (int) par1Vec3.yCoord, var7, par3, par4,
                            par5, par1Vec3, var8, var10));
        ...
        /**
         * Returns true when an entity could stand at a position, including
         * solid blocks under the entire entity. Args: xOffset, yOffset,
         * zOffset, entityXSize, entityYSize, entityZSize, originPosition, vecX,
         * vecZ
         *
         * @param   par1      DOCUMENT ME!
         * @param   par2      DOCUMENT ME!
         ...
         * @param   par4      DOCUMENT ME!
         * @param   par5      DOCUMENT ME!
         * @param   par6      DOCUMENT ME!
         * @param   par7Vec3  DOCUMENT ME!
         * @param   par8      DOCUMENT ME!
         * @param   par10     DOCUMENT ME!
         *
         * @return  true when an entity could stand at a position, including
         *          solid blocks under the entire entity.
         */
        private boolean isSafeToStandAt(int    par1,
                                        int    par2,
                                        int    par3,
                                        int    par4,
                                        int    par5,
            ...
            if (!this.isPositionClear(var12, par2, var13, par4, par5, par6, par7Vec3, par8,
                        par10)) {
                return false;
            } else {
        ...
        /**
         * Returns true if an entity does not collide with any solid blocks at
         * the position. Args: xOffset, yOffset, zOffset, entityXSize,
         ...
         * @param   par10     DOCUMENT ME!
         *
         * @return  true if an entity does not collide with any solid blocks at
         *          the position.
         */
        private boolean isPositionClear(int    par1,
                                        int    par2,
                                        int    par3,
                                        int    par4,
                                        int    par5,
                ...
                for (int var13 = par2; var13 < (par2 + par5); ++var13) {
                    ...
                    for (int var14 = par3; var14 < (par3 + par6); ++var14) {
                        ...
                        if (((var15 * par8) + (var17 * par10)) >= 0.0D) {
                            ...
                            if ((var19 > 0) &&
                                    !Block.blocksList[var19].getBlocksMovement(this.worldObj, var12,
                                        var13, var14)) {
                                return false;
                            }
                        }
